...
### 步骤 2：使用 `ScryptProvider` 连接 signer

将 signer 连接到 `ScryptProvider`，这是使用 sCrypt 服务所需的[provider](../how-to-deploy-and-call-a-contract/how-to-deploy-and-call-a-contract.md#provider)。
...
通常您可以从合约的创建者那里获取合约的 ID，创建者会公开它以便其他人可以与之交互。
...
- `spentUtxo`: 事件中花费的合约指定的 utxo。

- `contractId`: 事件所属的合约 ID。

- `spentBy`: 事件来自的合约调用交易的指定输入索引。

- `createdInSpentTxOutputs`: 如果是有状态合约，则在花费交易中新生成的合约 utxo。
